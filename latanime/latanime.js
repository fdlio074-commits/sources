function searchResults(html) {
    ...
    while ((containerMatch = containerRegex.exec(html)) !== null) {
        ...
        // Extraer enlace
        const linkRegex = /<a[^>]*href="(https:\/\/latanime\.com\/(?:anime|pelicula)\/[^"]+)"[^>]*>/i;
    ...
    if (results.length === 0) {
        const linkRegex = /<a[^>]*href="(https:\/\/latanime\.com\/(?:anime|pelicula)\/[^"]+)"[^>]*>[\s\S]*?<img[^>]*src="([^"]+)"[^>]*>[\s\S]*?<\/a>/gi;
...
function extractEpisodes(html) {
    ...
    const epRegex = /<a[^>]*href="(https:\/\/latanime\.com\/(?:anime|ver)\/[^"]+\/(?:ep|capitulo|episodio)[-\\d]+)"[^>]*>[\s\S]*?<\/a>/gi;
    let match;
    ...
    if (episodes.length === 0) {
        const epDivRegex = /<(?:div|span)[^>]*class="[^"]*(?:episodio|ep|capitulo)[^"]*"[^>]*>[\s\S]*?<a[^>]*href="(https:\/\/latanime\.com\/(?:anime|ver)\/[^"]+\/(?:ep|capitulo|episodio)[-\\d]+)"[^>]*>/gi;
